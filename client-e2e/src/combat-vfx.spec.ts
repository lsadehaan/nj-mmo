...
test('melee combat increments VFX hook counters', async ({ page }, testInfo) => {
  ...
  const sawTargetRing = await page.evaluate(() => window.__GAME_STATE__.vfx.targetRingVisible);
  expect(sawTargetRing).toBe(true);
    ...
    .poll(
      async () =>
        page.evaluate((mobId) => {
          ...
          window.__attack__?.();
          return state.vfx.meleeHitCount;
        }, target.id),
